...
const LogsTable = () => {
  ...
  const columns = [
    ...
    // 根据环境变量控制是否显示"提示"列
    ...(process.env.REACT_APP_SHOW_PROMPT_TOKENS === "true"
      ? [
          {
            title: "提示",
            dataIndex: "prompt_tokens",
            render: (text, record, index) => {
              return record.model_name.startsWith(
                "mj_"
              ) ? null : record.type === 0 || record.type === 2 ? (
      ...
      : []),
    // 根据环境变量控制是否显示"补全"列
    ...(process.env.REACT_APP_SHOW_COMPLETION_TOKENS === "true"
      ? [
          {
            title: "补全",
    ...
    // 根据环境变量控制是否显示"详情"列
    ...(process.env.REACT_APP_SHOW_CONTENT_DETAIL === "true"
      ? [
          {
            title: "详情",
            dataIndex: "content",
  ...
  const copyTokenInfo = (e) => {
    ...
    const info = `令牌总额: ${
      balance === 100000000 ? "无限" : `${balance.toFixed(3)}`
    }
剩余额度: ${
      balance === 100000000 ? "无限制" : `${(balance - usage).toFixed(3)}`
    }
            ...
            <Panel
              ...
              extra={
                <div style={{ display: "flex", alignItems: "center" }}>
                  <Tag shape="circle" color="green" style={{ marginRight: 5 }}>
                    计算汇率：$1 = 50 0000 tokens
                  </Tag>
                  <Button
                    icon={<IconDownload />}
                    theme="borderless"
